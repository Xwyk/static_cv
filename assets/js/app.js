...
function showSection(sectionId) {
  const section = document.getElementById(sectionId) || document.querySelector(`.${sectionId}-grid`)?.closest('.section');
  if (section) {
...
function displayInfos(data) {
  ...
  if (jobTitleEl) {
    jobTitleEl.textContent = data.position;
    jobTitleEl.classList.add('loaded');
  }
  if (locationEl) {
  ...
  if (educationEl) {
    educationEl.textContent = `🎓 ${data.degree}`;
    educationEl.classList.add('loaded');
  }
  if (data.description && descriptionEl) {
    descriptionEl.textContent = data.description;
    descriptionEl.classList.add('loaded');
  }
  if (data.photo) {
...
function displayDiplomes(diplomes) {
  const container = document.querySelector('.diplomes-grid');
  if (!container) return;
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <p class="card-subtitle">${d.school}</p>
        </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          ${p.techno ? `<p class="card-subtitle">${p.techno}</p>` : ''}
        </div>
...
function displayCompetences(competences) {
  ...
  const html = competences
    .map(comp => {
      const levelDots = Array.from({ length: 5 }, (_, i) =>
        `<span class="level-dot ${i < comp.level ? 'filled' : ''}"></span>`
      ).join('');

      return `
    <div class="competence-card">
      ${comp.icon ? `<div class="competence-icon">${comp.icon}</div>` : ''}
      <h3 class="competence-name">${comp.name}</h3>
      <div class="competence-level">${levelDots}</div>
    </div>
  `;
    })
    .join('');
...
function displayPassions(passions) {
  ...
  const html = passions
    ...
    <div class="passion-card">
      ${passion.icon ? `<div class="passion-icon">${passion.icon}</div>` : ''}
  ...
  container.innerHTML = html;
  showSection('passions');
}

function displayContact(liens) {
  ...
  const html = liens
    .map(item => {
      const icon = item.icon?.startsWith('http') 
        ? `<img src="${item.icon}" alt="${item.label}" class="contact-icon-img">`
        : `<div class="contact-icon">${item.icon}</div>`;
      
      return `
    <a href="${item.url}" class="contact-card" target="_blank" rel="noopener noreferrer">
      ${icon}
...
async function loadInfos() {
  try {
    ...
    console.error('Infos:', error.message);
  }
}

function loadRestData() {
  const dataConfigs = [
    { file: 'experiences', fn: displayExperiences },
    { file: 'diplomes', fn: displayDiplomes },
    { file: 'projets', fn: displayProjets },
    { file: 'applications', fn: displayApplications },
    { file: 'competences', fn: displayCompetences },
    { file: 'passions', fn: displayPassions },
    { file: 'contact', fn: displayContact }
  ];

  dataConfigs.forEach(config => {
    fetch(`assets/data/${config.file}.json`)
      .then(response => (response.ok ? response.json() : Promise.reject(new Error(`HTTP ${response.status}`))))
      .then(data => config.fn(data))
      .catch(error => console.error(config.file, error.message));
  });
}
